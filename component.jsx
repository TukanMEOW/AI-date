...
const DatingApp = () => {
  ...
  // Функция для динамической загрузки Telegram скрипта
  const loadTelegramScript = useCallback(() => {
    return new Promise((resolve) => {
      // Проверяем есть ли уже Telegram WebApp
      ...
      // Проверяем URL параметры - если есть tgWebAppData, то это точно Telegram
      const url = window.location.href;
      const hasTelegramParams = url.includes('tgWebAppData=') || url.includes('tgWebAppVersion=');
      
      if (hasTelegramParams) {
        console.log('🔍 Обнаружены параметры Telegram в URL, парсим данные...');
        ...
        try {
          ...
          if (tgWebAppData) {
            ...
            // Создаем мок Telegram WebApp объекта
            const mockTelegram = {
              WebApp: {
                ...
                HapticFeedback: {
                  impactOccurred: (style) => console.log('🤖 Mock HapticFeedback:', style)
                }
              }
            };
      ...
      if (existingScript) {
        console.log('🔄 Скрипт Telegram уже загружен, ждем инициализации...');
      ...
      console.log('📥 Динамическая загрузка Telegram WebApp скрипта...');
    ...
    const initializeApp = async () => {
      ...
      try {
        ...
        if (tg) {
          ...
          console.log('📱 InitData доступны:', !!tg.initData);
  ...
  const authenticateUser = async (tg) => {
    try {
      ...
      if (tg && tg.initData) {
        ...
        console.log('📱 InitData получены из WebApp API');
      } else {
      ...
      if (tg && tg.initDataUnsafe?.user) {
        console.log('🔄 Используем fallback авторизацию с данными Telegram');
        const fallbackUser = {
        ...
        // Полный fallback для тестирования
